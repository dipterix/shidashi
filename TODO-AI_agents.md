...
Expose shidashi Shiny tools to external AI agents (e.g., VS Code
Copilot) via the Model Context Protocol (MCP) Streamable HTTP transport.
The MCP endpoint lives inside the same httpuv process as the Shiny app —
...
The MCP endpoint is attached via `register_mcp_route()`, which wraps the
existing `shinyApp$httpHandler` produced by
[`shiny::shinyAppDir()`](https://rdrr.io/pkg/shiny/man/shinyApp.html)
and injects the MCP handler in front of it.
...
- `register_session_mcp(session)` — stores entry, registers
  `onSessionEnded` callback for automatic cleanup; called from
  `modules.R`
- `mcp_unregister_session(session)` — removes by token
- `mcp_sweep_closed_sessions()` — defensive `isClosed()` sweep on every
  MCP request (belt + suspenders)
- `mcp_get_shiny_entry(token)` — O(1) registry lookup, returns entry or
  NULL if not found / closed
...
`DELETE /mcp` → 200 [`{}`](https://rdrr.io/r/base/Paren.html) +
unregisters MCP session binding - `GET /mcp` → 200 info JSON - other
methods / paths → delegated to original handler
...
constructor and `setup_mcp_proxy()`. The `setup_mcp_proxy()` function
writes port records and copies `inst/mcp-proxy/shidashi-proxy.mjs` to
the user cache; called from
[`render()`](https://dipterix.org/shidashi/reference/render.md) on every
launch.

#### `R/render.R` (modified)

[`render()`](https://dipterix.org/shidashi/reference/render.md) resolves
or picks a random port, calls `setup_mcp_proxy()`, then uses
`register_mcp_route(shiny::shinyAppDir(root_path))` to attach the MCP
handler before running the app:
...
Both the interactive path and the RStudio job path use this pattern. The
`ui.R` inside `root_path` is rewritten at launch time to inject
...
body is prepended with a block that calls
`register_session_mcp(session)` and then populates `entry$tools` from
the module’s tool generators.
...
Every MCP tool (and the tunnel itself) must have a non-official dev test
under `adhoc/mcp/`. These are **not** part of the R package test suite
(`tests/`) — they require a live running app and are run manually from a
terminal.
...
When a new MCP tool is added: 1. Add at least one test block to
`adhoc/mcp/test_mcp_tools.sh` covering the happy path and one error
path. 2. Document expected output in a comment block above the curl
call. 3. Run `adhoc/mcp/test_mcp_tools.sh` before committing to verify
no regression.
...
`mcp-handler.R`. They live as
[`mcp_wrapper()`](https://dipterix.org/shidashi/reference/mcp_wrapper.md)
generators in `agents/tools/hello_world.R` and
`agents/tools/get_shiny_input_values.R` and are instantiated with the
live Shiny session at bind time.
...
No `token` parameter is exposed to the AI agent — the session is already
resolved at registration time. The `Mcp-Session-Id` header is generated
on `initialize` and echoed on every response. On a successful
`register_shinysession` call, an SSE response with a
`notifications/tools/list_changed` notification is emitted so the client
refreshes its tool catalogue.

### Verification

See `adhoc/mcp/test_mcp_tools.sh` for the full test sequence. Steps that
require a browser tab are skipped automatically when no sessions are
registered, with a hint to open the app URL.
...
per-session system. Tool authors write
[`mcp_wrapper()`](https://dipterix.org/shidashi/reference/mcp_wrapper.md)
generators in `agents/tools/*.R`. Modules opt into root-level tools via
`agents/agent.yaml`. The MCP session binds to a Shiny session via a
built-in `register_shinysession` tool — after binding, per-session tools
become callable.
...
Built-in infrastructure tools have no prefix: `list_shinysessions`,
`register_shinysession`, `get_session_info`.
...
The agent calls `register_shinysession(token)` to bind. Once bound, all
per-session tools for that Shiny session become available. The agent can
switch sessions by calling `register_shinysession` again.

### Package files

| File               | Status   | Purpose                                                                                                                            |
|--------------------|----------|------------------------------------------------------------------------------------------------------------------------------------|
| `R/mcp-wrapper.R`  | new      | [`mcp_wrapper()`](https://dipterix.org/shidashi/reference/mcp_wrapper.md) constructor + `is_mcp_wrapper()`                         |
...
The `mcp_wrapper` is a closure factory. The 5-arg generator receives the
live Shiny session context. It returns one
[`ellmer::ToolDef`](https://ellmer.tidyverse.org/reference/tool.html) or
a named list of them. The AI agent never sees these 5 args — they are
enclosed at session registration time.
...
`R/mcp-wrapper.R` with
[`mcp_wrapper()`](https://dipterix.org/shidashi/reference/mcp_wrapper.md),
`is_mcp_wrapper()`

`R/mcp-discover.R` with discovery + instantiation + schema conversion

`register_mcp_tools()` exported function

Session binding state (`mcp_session_bindings()` fastmap)

`register_shinysession`, `get_session_info` built-in MCP tools

Dynamic `tools/list` and `tools/call` with binding enforcement

Template: root `agents/tools/` + `modules/demo/agents/`

Updated `adhoc/mcp/test_mcp_tools.sh`
...
Each skill becomes one tool named `skill__{namespace}__{sanitized_name}`
that dispatches on an `action` enum (`readme` / `reference` / `script`).
A server-side gate rejects premature script/reference calls with a
condensed auto-generated summary (~200 tokens) embedded in the error —
teaching the AI in the rejection itself, costing only 1 retry when the
AI skips readme.
...
Per the Anthropic spec, the **folder name is the canonical skill name**.
No frontmatter `name` override. Discovery is a direct lookup — no
directory iteration needed. Missing folders are silently dropped.

    skill__{namespace}__{folder_name}

where `namespace` comes from `session$ns(NULL)` (e.g., `"demo"`). This
parallels the `tool__{namespace}__{tool_name}` convention from Phase 3.
...
- **`action=readme`**: Sets `readme_unlocked <<- TRUE` in closure.
  Returns SKILL.md body + runtime info. Full instructions — loaded only
  on-demand.
- **`action=reference`**: Takes `file_name`, optional `pattern` (grep),
  `line_start`, `n_rows`. Returns paginated file content. Gated behind
  readme.
- **`action=script`**: Takes `file_name`, `args`, optional `envs`. Runs
  via [`processx::run()`](http://processx.r-lib.org/reference/run.md)
  with interpreter resolved from file extension (`.R` → Rscript, `.py` →
  python3, `.sh` → bash). Supports optional `runtime` config for
  virtualenv activation etc. Gated behind readme.
...
| `R/skill-runner.R`  | new      | `build_script_command()`, script execution via [`processx::run()`](http://processx.r-lib.org/reference/run.md)                                                                                                                                                     |
| `R/skill-wrapper.R` | new      | [`skill_wrapper()`](https://dipterix.org/shidashi/reference/skill_wrapper.md) constructor — takes skill dir path, returns closure with class `shidashi_skill_wrapper` that produces [`ellmer::tool`](https://ellmer.tidyverse.org/reference/tool.html) per session |
...
[`ellmer::tool`](https://ellmer.tidyverse.org/reference/tool.html) that
dispatches on `action`. No R6 — pure function enclosure.
...
Create `agents/skills/greet/` (root-level): - `SKILL.md` with
Anthropic-compliant frontmatter - `scripts/greet.R` — simple Rscript
...
`R/skill-parse.R` with `parse_skill_md()`, `discover_references()`,
`discover_scripts()`

`R/skill-runner.R` with `build_script_command()` + processx execution

`R/skill-wrapper.R` with
[`skill_wrapper()`](https://dipterix.org/shidashi/reference/skill_wrapper.md)
constructor
...
Updated `adhoc/mcp/test_mcp_tools.sh` with skill tests (gate, readme,
reference, script)
...
[`render()`](https://dipterix.org/shidashi/reference/render.md) calls
`setup_mcp_proxy()` automatically and prints the config snippet. The
proxy (`inst/mcp-proxy/shidashi-proxy.mjs`) is a Node.js stdio-to-HTTP
bridge installed in the user cache. Add to `.vscode/mcp.json`:
...
The proxy auto-discovers the most recent port record written to the user
cache when no port arg is given.

------------------------------------------------------------------------

## Phase 6: In-App Chatbot Drawer

**Goal**: Add an AI chatbot panel inside the drawer that uses
[`ellmer::Chat`](https://ellmer.tidyverse.org/reference/Chat.html) with
`shinychat` to let users interact with the active module’s tools
...
1.  **ellmer direct integration, not MCP HTTP**: The chatbot calls
    ...
    exclusively for **external** MCP clients (e.g. VS Code Copilot). The
    in-app chatbot never routes through HTTP.

2.  **One session token per iframe**: Each iframe (module page) or root
    page has exactly **one** Shiny session and one `session$token` —
    analogous to `private_id`. In JS, `_sessionToken` is a single
    string, not a map. When a module registers via
    `register_session_mcp()`, it sends its token to JS via
    `shidashi.register_module_token`. JS stores it as a single value
    (overwritten on each module activation).

3.  **Token included in `@shidashi_active_module@`**: The
    `_reportActiveModule(moduleId)` method now includes the token in the
    Shiny input: `{ module_id, token, timestamp }`. This lets R-side
    code
    (e.g. [`chatbot_server()`](https://dipterix.org/shidashi/reference/chatbot_server.md))
    look up the active module’s tools from the MCP registry keyed by
    token.

4.  **JS fires event only — R opens the drawer**: When the chatbot FAB
    button is clicked, JS broadcasts a `shidashi-event` of type
    `chatbot` with `{ module_id, token }`. It does **not** open the
    drawer or switch tabs from JS. The R-side
    [`chatbot_server()`](https://dipterix.org/shidashi/reference/chatbot_server.md)
    observes this event, calls `shidashi::drawer_open(session)`, and
    handles all chat logic. This way, if the chatbot is disabled (no
    shinychat / `options(shidashi.chatbot = FALSE)`), the button simply
    does nothing.

### Design Decisions

- **Per-module chat sessions**: Separate
  [`ellmer::Chat`](https://ellmer.tidyverse.org/reference/Chat.html)
  objects per module, stored in `.__shidashi_globals__.$chat_sessions`
  (fastmap keyed by module_id). Switching modules switches the
  conversation context.
- **Tabbed drawer**: The drawer gets Bootstrap 5 nav-tabs: **Settings**
  tab (existing `drawer_ui()` content) and **Chat** tab
  ([`chatbot_ui()`](https://dipterix.org/shidashi/reference/chatbot_ui.md)
  content).
- **Provider via [`options()`](https://rdrr.io/r/base/options.html)**:
  `options(shidashi.chat_provider = "anthropic")` with optional
  `shidashi.chat_model` and `shidashi.chat_base_url`. The
  [`init_chat()`](https://dipterix.org/shidashi/reference/init_chat.md)
  function dispatches to the appropriate `ellmer::chat_*()`.
- **Conditional on shinychat**: Feature disabled when
  `options(shidashi.chatbot = FALSE)` or `shinychat` is not installed.
  [`chatbot_ui()`](https://dipterix.org/shidashi/reference/chatbot_ui.md)
  returns empty `tagList()`;
  [`chatbot_server()`](https://dipterix.org/shidashi/reference/chatbot_server.md)
  no-ops.
- **FAB button**: A `.btn-chatbot` element added to
  `.shidashi-back-to-top` (via
  [`back_top_button()`](https://dipterix.org/shidashi/reference/back_top_button.md)),
  with `data-shidashi-action="chatbot-toggle"`.
...
**`R/widgets.R`** — Modify
[`back_top_button()`](https://dipterix.org/shidashi/reference/back_top_button.md)
to conditionally append a chatbot button with
`data-shidashi-action="chatbot-toggle"` and an `fa-robot` icon. Only
rendered when `getOption("shidashi.chatbot", TRUE)` and `shinychat` is
available.
...
`{ module_id: this._activeModuleId, token: this._sessionToken }`. **Do
NOT open the drawer or switch tabs from JS.** R handles that.

### Sub-phase 6c: Tabbed Drawer

**`index.html`** — Restructure `.shidashi-drawer` content into two BS5
nav-tabs: Settings and Chat:
...
`init_chat(provider_name, model, base_url, module_id)` — reads options,
creates
[`ellmer::Chat`](https://ellmer.tidyverse.org/reference/Chat.html), sets
`system_prompt` from `module_agent_config`. - Returns bare `Chat` (tools
bound lazily by
[`chatbot_server()`](https://dipterix.org/shidashi/reference/chatbot_server.md)).

### Sub-phase 6f: `chatbot_ui()` and `chatbot_server()`

**`chatbot_ui(id = "shidashi-chatbot")`**: Guard +
[`shinychat::chat_ui()`](https://posit-dev.github.io/shinychat/r/reference/chat_ui.html).

**`chatbot_server(id = "shidashi-chatbot", session)`**: - Observes
`@shidashi_event@` type `"chatbot"`: - Extracts `module_id`, `token` -
Opens drawer via `shidashi::drawer_open(session)` - Creates or retrieves
`Chat` from `chat_sessions` (keyed by module_id) - Finds tools from
`mcp_session_registry()$get(token)$tools` - Calls
`chat$register_tools(tools)` (ellmer direct, no HTTP) - Switches chat
context in the UI - Observes `input${id}_user_input`: - Streams via
`chat$stream_async()` →
...
[`shidashi::chatbot_server()`](https://dipterix.org/shidashi/reference/chatbot_server.md).
Generated `R/common.R` includes
...
[`chatbot_server()`](https://dipterix.org/shidashi/reference/chatbot_server.md)
swaps the `Chat` context: saves current turns (already in Chat object),
loads target module’s Chat, replays history.

### Files to Create/Modify

| File                | Action     | Description                                                                                                                                                                                                                     |
|---------------------|------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `R/chatbot.R`       | **Create** | [`init_chat()`](https://dipterix.org/shidashi/reference/init_chat.md), [`chatbot_ui()`](https://dipterix.org/shidashi/reference/chatbot_ui.md), [`chatbot_server()`](https://dipterix.org/shidashi/reference/chatbot_server.md) |
...
| `R/widgets.R`       | Modify     | Add chatbot FAB to [`back_top_button()`](https://dipterix.org/shidashi/reference/back_top_button.md) (already done)                                                                                                             |
...
(all modes) or a list of mode names (e.g. `["Plan", "Executing"]`). Add
a UI mode selector in the chatbot header, a built-in `ask_user` MCP tool
for interactive user prompts, and automatic confirmation prompts for
tools/scripts annotated with `category: [destructive]`. All async via
`promises` + `later`.
...
New fastmap inside `.__shidashi_globals__.`, keyed by `module_id`. Each
value: `list(current_mode, modes, default_mode)`.
...
Choices populated from `agent_conf$modes` (`name` field), preselected to
`agent_conf$parameters$default_mode`.
...
Filter per-session tools by current mode before returning schema to MCP
clients. External clients only see tools available in the active mode.
...
Register `shidashi.ask_user`: - Render inline prompt in chatbot area:
question text + choice buttons (or text input if no choices) - On
response:
`Shiny.setInputValue(params.input_id, { request_id, response }, { priority: "event" })` -
...
`"destructive"`. Use session getter from annotation to invoke the same
confirmation pattern. Returns promise.
...
`module_agent_modes` in init-app globals

Raw `enabled` preservation in tool/skill annotations

`is_tool_enabled_for_mode()`, `is_script_enabled_for_mode()` helpers

Mode selector dropdown in chatbot header UI

Mode change observer + `bind_tools_for_mode()`

Mode guard in `mcp_handle_tools_call()`

Mode-aware `mcp_handle_tools_list()`

`ask_user` built-in MCP tool (R + JS)

Destructive auto-confirmation in MCP tool calls

Destructive auto-confirmation for skill scripts (via per-script category
check)

Per-conversation mode persistence

`npm run build` for JS/SCSS assets
