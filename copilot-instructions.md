...
Per-application mutable state (session registries, per-module input
specs, etc.) lives in a `.__shidashi_globals__.` environment created by
[`shidashi::init_app()`](https://dipterix.org/shidashi/reference/init_app.md)
in `global.R`. Internal helpers obtain it via `get_shidashi_globals()`
which walks the environment chain with `inherits = TRUE`.

- **Never add package-level mutable state** (e.g. `local({})` singletons
  or environments assigned at package load time in `R/*.R`). This
  pattern risks memory leaks across Shiny app restarts, violates CRAN
  policy, and makes testing unreliable. All mutable registries must be
  scoped to the application via `.__shidashi_globals__.`.
- `global.R` is generated by
  [`render()`](https://dipterix.org/shidashi/reference/render.md)
  (copied from `inst/global.R`). It is listed in each template’s
  `.gitignore` because it is a build artifact.

## Editing Rules

- **Never remove comments** from R source files or template files.
  Comments (especially commented-out code and `#---` section headers)
  are intentional documentation for users. When editing
  [`writeLines()`](https://rdrr.io/r/base/writeLines.html) blocks in
  `barebone.R`, preserve all comment lines exactly as they appear in the
  corresponding template files
  (e.g. `inst/builtin-templates/*/R/common.R`).
